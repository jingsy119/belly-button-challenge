...
function updateAll(entryNum){
  ...
  d3.json(url).then(function(data){
    ...
    ///////////////////Updates the Demographic Info /////////////////
    d3.selectAll("tr").remove();
    let demoBody = d3.select("#sample-metadata");
    let obKeys = Object.keys(data.metadata[entryNum]);
    let obValues = Object.values(data.metadata[entryNum]);
    for (x = 0; x < obKeys.length; x++){
      // Append one table row `tr` to the table body
      demoBody.append("tr").text(`${obKeys[x]}: ${obValues[x]}`)
    }
